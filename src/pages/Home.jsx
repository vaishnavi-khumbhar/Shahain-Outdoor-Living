...
export default function Home() {
  ...
  const heroImages = [media.heroSlide1, media.heroSlide2, media.heroSlide3];
    ...
    <>
      ...
      <section className="py-10 sm:py-16 lg:py-24">
        <div className="container-shahain">
          ...
          <Stagger className="grid grid-cols-2 gap-3 sm:grid-cols-2 sm:gap-5 lg:grid-cols-4">
            {applications.map((application) => {
              ...
              return (
                <StaggerItem key={application.label}>
                  <div className="group relative min-h-[200px] overflow-hidden border border-sand/60 bg-navy sm:min-h-[260px] lg:min-h-[240px]">
                    {/* IMAGE */}
                    ...
                    <div className="absolute inset-0 bg-navy/45 transition-all duration-500 group-hover:bg-navy/70" />

                    {/* CONTENT */}

                    <div className="relative z-10 flex h-full min-h-[200px] flex-col items-center justify-end p-4 text-center sm:min-h-[260px] sm:p-5 lg:min-h-[240px]">
                      <div className="mb-3 flex h-11 w-11 items-center justify-center border border-champagne/60 bg-navy/45 backdrop-blur-sm transition-all duration-300 group-hover:border-champagne group-hover:bg-champagne sm:mb-4">
                        <Icon
                          className="h-6 w-6 text-champagne transition-colors duration-300 group-hover:text-navy"
                          strokeWidth={1.4}
                        />
                      </div>

                      <span className="font-body text-[12px] font-semibold uppercase tracking-[0.1em] text-ivory sm:text-[12px] sm:tracking-[0.12em]">
                        {application.label}
                      </span>
                    </div>
                  </div>
      ...
      <section className="relative overflow-hidden bg-navy py-10 sm:py-16 lg:py-20">
              ...
              return (
                <StaggerItem key={item.title}>
                  <motion.div
                    ...
                    className="group relative h-full overflow-hidden rounded-sm border border-ivory/15 bg-ivory/[0.03] p-6 text-center backdrop-blur-sm transition-all duration-500 hover:border-champagne/60 hover:bg-ivory/[0.07] hover:shadow-[0_20px_60px_rgba(0,0,0,0.25)] sm:p-8"
                  >
                    ...
                    <div className="relative mx-auto flex h-24 w-24 items-center justify-center sm:h-28 sm:w-28">
                      ...
                      <motion.div
                        ...
                        className="relative flex h-[72px] w-[72px] items-center justify-center rounded-full border border-champagne/30 bg-champagne/[0.08] transition-all duration-500 group-hover:bg-champagne/15 group-hover:shadow-[0_0_30px_rgba(184,155,98,0.15)] sm:h-20 sm:w-20"
                      >
                        <Icon
                          className="h-9 w-9 text-champagne transition-transform duration-500 group-hover:scale-110 sm:h-10 sm:w-10"
                          strokeWidth={1.35}
                        />
                      </motion.div>
                    </div>
                    ...
                    <p className="relative mt-3 font-body text-[16px] leading-[1.65] text-ivory/70 transition-colors duration-300 group-hover:text-ivory/85 sm:mt-4 sm:text-sm sm:leading-relaxed sm:text-ivory/55 sm:group-hover:text-ivory/70">
                      {item.description}
                    </p>
      ...
      <section className="border-t border-sand/50 py-10 sm:py-20 lg:py-28">
        ...
        <div className="container-shahain mt-8 flex justify-center sm:mt-10">
      ...
      <section className="bg-deep-navy py-10 sm:py-16 lg:py-20">
        <div className="container-shahain flex flex-col items-center gap-5 text-center sm:gap-8">
          <Reveal direction="up">
            <Quote className="h-10 w-10 text-champagne" strokeWidth={1} />
          </Reveal>

          <Reveal direction="up" delay={0.1}>
            <p className="max-w-xl text-balance font-heading text-[26px] italic leading-[1.5] text-ivory/85 sm:text-2xl sm:text-ivory/80 lg:text-3xl">
          ...
          <Reveal direction="up" delay={0.2}>
            <p className="max-w-md font-body text-[16px] leading-[1.65] text-ivory/65 sm:text-sm sm:text-ivory/50">
      ...
      <section className="py-10 sm:py-16 lg:py-20">
        <div className="container-shahain">
          ...
          <Stagger className="grid grid-cols-1 gap-x-8 gap-y-10 sm:grid-cols-2 sm:gap-y-12 lg:grid-cols-3 lg:gap-y-14">
              ...
              return (
                <StaggerItem key={post.slug}>
                  <Link to={`/blog/${post.slug}`} className="group block">
                    <ImageFrame
                      src={post.image || postCategory?.galleryImage || postCategory?.heroImage}
                      alt={post.title}
                      ratio="aspect-[4/3]"
                      label={post.category}
                      className="transition-transform duration-700 group-hover:scale-[1.03]"
                    ...
                    <div className="mt-4 flex items-start justify-between gap-4 border-t border-sand/60 pt-4 sm:mt-5 sm:pt-5">
                      <div>
                        <span className="font-body text-xs uppercase tracking-[0.16em] text-champagne sm:text-[11px] sm:tracking-[0.2em]">
                          {post.category} &middot; {post.readTime}
                        </span>

                        <h3 className="mt-2 font-heading text-xl leading-snug text-navy sm:text-lg">
                          {post.title}
                        </h3>
                      </div>

                      <ArrowUpRight
                        className="mt-1 h-5 w-5 shrink-0 text-champagne transition-transform duration-300 group-hover:-translate-y-1 group-hover:translate-x-1"
                        strokeWidth={1.5}
                      />
                    </div>
                  </Link>
          ...
          <Reveal direction="up" className="mt-10 flex justify-center sm:mt-16">
